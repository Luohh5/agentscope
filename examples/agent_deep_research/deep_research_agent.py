...
_LOG_PATH = os.path.join(_LOG_DIR, f"log_{datetime.now().strftime('%y%m%d%H%M%S')}.md")
...
class DeepResearchAgent(ReActAgent):
    ...
    async def reply(
        ...
        # Identify the expected output checklist and generate a global plan at first
    ...
    async def _follow_up(
        ...
        if len(self.current_subtask) < self.max_depth:
            ...
            expansion_inst = self.prompt_dict["expansion_inst"].format_map(
                {
                    "objective": tool_call['input']['query'],
            ...
            if follow_up_subtask.get("need_more_information", False):
                expansion_response_msg = Msg(
                    "assistant",
                    follow_up_subtask.get('reasoning', "I need more information."),
                    role="assistant",
                )
                urls = follow_up_subtask.get("url", None)
                logger.info(f"Reading {urls}")
                ...
                if not follow_up_response.get("is_sufficient", True):
                    ...
                    logger.info(f"Figuring out {subtasks}")
                    intermediate_report = (
    ...
    async def summarize_intermediate_results(self) -> ToolResponse:
        ...
        for item in self.intermediate_memory:
            if isinstance(item.content, str):
                ...
                logger.warning(f"Unknown content type: {type(item.content)}!")
                continue
        ...
        intermediate_report_path = os.path.join(
            ...
            f"{self.report_path_based}_inprocess_report_{self.report_index}.md",
        )
        self.report_index += 1
        params = {
        ...
        logger.info(
            f"Storing the intermediate findings:\n{intermediate_report}",
        )
        if (
    ...
    async def _generate_deepresearch_report(
            ...
            for index in range(self.report_index):
                params = {
                    "file_path": os.path.join(
                        self.tmp_file_storage_dir,
                        f"{self.report_path_based}_inprocess_report_{index + 1}.md",
                    ),
                }
        ...
        logger.info(f"The final Report is generated:\n{final_report_content}")
    ...
    async def reflect_failure(self) -> ToolResponse:
        ...
        if reflection.get("rephrase_subtask", False) and reflection[
            ...
            if len(self.current_subtask) <= self.max_depth:
                ...
                self.current_subtask.append(
                    SubTaskItem(
                        objective=reflection["decompose_subtask"].get(
                            "failed_subtask",
                            None,
                        ),
                    ),
