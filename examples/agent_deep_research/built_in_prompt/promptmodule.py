...
class WebExtraction(BaseModel):
    ...
    reasoning: str = Field(
        description="The reasoning for your decision, including a summary of evidence and logic for whether more information is needed.",
    )
    need_more_information: bool = Field(
        description="Whether more information is needed.",
    )
    title: str = Field(
        description="Title of the identified search result snippet that requires further extraction, or an empty string if not applicable.",
    )
    url: str = Field(
        description="Direct URL to the original search result requiring further extraction, or an empty string if not applicable.",
    )
    subtask: str = Field(
        description="Actionable description of the follow-up task to obtain needed information, or an empty string if not applicable.",
    )
        ...
        json_schema_extra={
            "additionalProperties": {
                ...
                "properties": {
                    ...
                    "rephrased_plan": {
                        "type": "string",
                        "description": "Information about whether the failed subtask "
                                       "requires decomposition, and the failed subtask "
                                       "itself if needed.",
                    },
                }
            }
